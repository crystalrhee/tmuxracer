...
const { init, GameLoop, Sprite, Text, initKeys, keyPressed } = kontra;
...
const hotkeyCombos = [
    'ctrl+a', 'ctrl+s', 'ctrl+z', 'ctrl+c', 'ctrl+v', 'ctrl+x',
    'ctrl+f', 'ctrl+n', 'ctrl+o', 'ctrl+p', 'ctrl+w', 'ctrl+t',
    'shift+a', 'shift+b', 'shift+c', 'shift+d', 'shift+e',
    'alt+tab', 'alt+f4', 'alt+enter',
    'ctrl+shift+n', 'ctrl+shift+t', 'ctrl+shift+i',
...
function spawnHotkey() {
    ...
    const handler = function(event, handler) {
...
function createGameLoop() {
    ...
    render: function() {
        ...
        if (gameState.fallingHotkeys.length === 0 && gameState.isRunning) {
            ...
            context.fillText('Waiting for hotkeys to spawn...', canvas.width / 2, canvas.height / 2);
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let kontraInit = init(canvasElement);
